...
app.get("/user/tweets/feed", authenticateToken, async (request, response) => {
  ...
  const getTweetsFeedQuery = `
        ...
        FROM 
            follower INNER JOIN tweet ON follower.following_user_id =tweet.user_id INNER JOIN ON
...
app.get("/tweets/:tweetId", authenticateToken, async (request, response) => {
  ...
  const userFollowersQuery=`
  SELECT * FROM follower INNER JOIN user ON user.user_id=
  follower.following_user_id
  WHERE follower.follower_user_id = ${user_id};`;

  const userFollowers.some(
    ...
    const tweetDetails =await db.get(getTweetDetailsQuery);  //tweet_id INNER JOIN reply ON reply.tweet_id=
...
app.get("/tweets/:tweetId/likes", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name, tweetId);
  const getLikedUsersQuery = ` SELECT *
...
app.get("/tweets/:tweetId/replies", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name, tweetId);
  const getRepliedUsersQuery = ` SELECT *
...
app.delete("/tweets/:tweetId/", authenticateToken, async (request, response) => {
  ...
  const selectUserQuery = `
SELECT * FROM tweet WHERE tweet.user_id=${user_id} AND tweet.tweet_id=
${tweetId};`;
     const tweetUser =

  await db.all(selectUserQuery);
